...
async function fetchUserProfile(username) {
  const query = `
    SELECT users.mail, userinfo.firstname, userinfo.lastname, userinfo.current_weight, userinfo.goal_weight, userinfo.dateofbirth, userinfo.height, userinfo.gender, userinfo.bodyFat, userinfo.activitylevel
    FROM users  
...
async function updateUserProfile(userData, userId) {
  const updateQuery = 'UPDATE userinfo SET firstname = ?, lastname = ?, current_weight = ?, goal_weight = ?, dateofbirth = ?, height = ?, gender = ?, bodyFat = ?, activitylevel = ? WHERE iduser = ?';
  await executeQuery(updateQuery, [userData.firstname, userData.lastname, userData.current_weight, userData.goal_weight, userData.dateofbirth, userData.height, userData.gender, userData.bodyFat, userData.activityLevel, userId]);
}
