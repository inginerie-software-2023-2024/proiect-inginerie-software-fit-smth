...
export const addUserMeal = (req, res) => {
    const { username, date, food, meal_type } = req.body;
    ...
    food.forEach(foodItem => {
        console.log(`Processing meal item for user: ${username}`, foodItem); 
        console.log(`At meal: ${meal_type}`)

        const query = "INSERT INTO user_meals (username, date, id_aliment, quantity, meal_type) VALUES (?, ?, ?, ?, ?)";
        db.query(query, [username, date, foodItem.id, foodItem.q, meal_type], (err, data) => {
...
export const getUserMeals = (req, res) => {
    ...
    const query = `SELECT um.*, a.* 
                    FROM user_meals um, alimente a 
                    WHERE um.username = ? AND a.id = um.id_aliment 
