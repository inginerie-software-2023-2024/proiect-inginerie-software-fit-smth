...
const UserProfileForm = ({ userData, onSaveChanges }) => {
 ...
 // Mapping from activity factor keys to labels
 const activityLevelMapping = {
  ...
  veryActive: 'Super active (very hard exercise/sports & physical job or 2x training)'
};

// Convert mapping to options format for dropdown
const activityLevelOptions = Object.entries(activityLevelMapping).map(([key, label]) => ({
  value: key,
  label: label
}));
