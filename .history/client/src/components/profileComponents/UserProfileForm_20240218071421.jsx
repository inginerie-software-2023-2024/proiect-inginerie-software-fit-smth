...
const UserProfileForm = ({ userData, onSaveChanges }) => {
  ...
  // Assuming handleSaveChanges is defined in UserProfileForm or a higher component
  const handleSaveChanges = async (editedData) => {
    try {
      ...
      if (response && response.success) {
        ...
        // Optionally, refresh data here or update state to reflect changes
      } else {
        // Handle failure (if your API response structure includes success status)
...
const UserProfileEditForm = React.memo(({ editedData, handleChange, renderInputFields, handleSaveChanges }) => {
  // Correcting the handleSubmit function
  const handleSubmit = async (e) => {
    e.preventDefault(); // This prevents the default form submission action

    // Asynchronously call handleSaveChanges and await its completion
    try {
      await handleSaveChanges(editedData); // Assuming handleSaveChanges expects editedData
      // Optional: Provide success feedback directly or via a state update
    } catch (error) {
      // Handle any errors, possibly updating the state to show an error message
    }
  };

  // Ensure your form and inputs are correctly set up
  return (
