...
function handleDeleteFood(foodToRemove) {
    ...
    function formatSignificantDigits(number, significantDigits = 2) {
        if (number === 0) return 0;
      
        const magnitude = Math.floor(Math.log10(Math.abs(number)));
        const scale = Math.pow(10, significantDigits - 1 - magnitude);
      ...
      const CalculationResult = ({ result, saveResult }) => (
        result ? (
            <div className='result'>
                ...
                <p>You ate <strong>{formatSignificantDigits(result.q)} grams</strong> today, which means:</p>
                <ul>
                    <li><strong>{formatSignificantDigits(result.cal)} calories</strong></li>
                    <li><strong>{formatSignificantDigits(result.carbs)} carbs</strong></li>
                    <li><strong>{formatSignificantDigits(result.fat)} fat</strong></li>
                    <li><strong>{formatSignificantDigits(result.protein)} protein</strong></li>
                </ul>
