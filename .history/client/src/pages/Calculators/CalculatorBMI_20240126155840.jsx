...
const BMICalculator = () => {
  ...
  const [feet, setFeet] = useState(''); // State variable for feet
  const [inches, setInches] = useState(''); // State variable for inches
  ...
  const handleInputChange = (e) => {
    ...
    // Update feet and inches based on height input
    if (id === 'height') {
      if (selectedUnit === 'imperial') {
        // Parse the value as an integer and split into feet and inches
        const heightValue = parseInt(value, 10);
        setFeet(Math.floor(heightValue / 12));
        setInches(heightValue % 12);
      } else {
        // Use empty strings for feet and inches if using metric units
        setFeet('');
        setInches('');
      }
    }
