    ...
    const UserFoodList = ({ userList, setUserList, handleDeleteFood }) => (
        <div className="search-result">
            {userList.length ? userList.map((food, index) => (
                <UserFoodItem food={food} index={index} setUserList={setUserList} userList={userList} handleDeleteFood={handleDeleteFood} key={food.id} />
    ...
    const CalculationResult = ({ result, saveResult }) => (
        result ? (
            <div className='result'>
                ...
                <p>You've eaten <strong>{parseFloat(result.q).toFixed(2)} grams</strong> today, which means:</p>
                <ul>
                    <li><strong>{parseFloat(result.cal).toFixed(2)} calories</strong></li>
                    <li><strong>{parseFloat(result.carbs).toFixed(2)} carbs</strong></li>
                    <li><strong>{parseFloat(result.fat).toFixed(2)} fat</strong></li>
                    <li><strong>{parseFloat(result.protein).toFixed(2)} protein</strong></li>
                </ul>
                ...
                <div className="meal-type-select">
                    ...
                    <div className="btn-group" role="group">
                        <button type="button" className={`btn btn-outline-secondary ${mealType === 'breakfast' ? 'active' : ''}`} onClick={() => setMealType('breakfast')}>Breakfast</button>
                        <button type="button" className={`btn btn-outline-secondary ${mealType === 'lunch' ? 'active' : ''}`} onClick={() => setMealType('lunch')}>Lunch</button>
                        <button type="button" className={`btn btn-outline-secondary ${mealType === 'dinner' ? 'active' : ''}`} onClick={() => setMealType('dinner')}>Dinner</button>
                        <button type="button" className={`btn btn-outline-secondary ${mealType === 'snack' ? 'active' : ''}`} onClick={() => setMealType('snack')}>Snack</button>
                    </div>
                </div>
            </div>
        ) : <span></span>
