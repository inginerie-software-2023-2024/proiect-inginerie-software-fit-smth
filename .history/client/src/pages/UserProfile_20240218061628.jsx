...
const UserProfile = () => {
  ...
  }, [selectedDate, username, refreshTrigger]); // Include refreshTrigger in the dependency array

  const handleSaveChanges = async (updatedData) => {
    const success = await updateUserData(username, updatedData, setUserData);
    if (success) {
      setRefreshTrigger(oldTrigger => oldTrigger + 1); // Update the refresh trigger to force re-fetch
    }
  };
...
async function updateUserData(username, updatedData, setData) {
  console.log(`Updating data for user: ${username}`, updatedData);
  try {
    ...
    if (response.data.Status === "Success") {
      console.log('Data updated successfully in backend, re-fetching user data...');
      fetchUserData(username, setData); // Refetch to ensure UI is up to 
    } else {
