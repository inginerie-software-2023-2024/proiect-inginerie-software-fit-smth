...
const DatePickerInput = ({ selectedDate, setSelectedDate }) => {
  // Function to format the selected date into a readable string
  const formatDate = (date) => {
    if (!date) return "Select a date"; // Placeholder text if no date is selected
  ...
  return (
    <div className="date-picker-container d-flex align-items-center">
      ...
      <span className="selected-date-display" style={{ marginRight: '10px' }}>{formatDate(selectedDate)}</span>
      <DatePicker
