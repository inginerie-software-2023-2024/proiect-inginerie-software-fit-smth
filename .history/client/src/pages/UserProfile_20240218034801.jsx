...
const UserProfile = () => {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const response = await onSaveChanges(editedData); // onSaveChanges should return the updated data or a success status
      if (response.status === "Success") {
        console.log('Changes successfully saved');
        fetchUserData(username, setUserData); // Refetch user data to update the state
        setEditMode(false);
...
async function fetchUserData(username, setData) {
  try {
    ...
    if (res.data.Status === "Success") {
      setData(res.data.data); // Ensure this updates the state with the latest data
    } else {
