...
const UserProfile = () => {
  ...
  async function getUserMealsData(username) {
    const response = await axios.get(`http://localhost:3001/food/getUserMeals/${username}`).then((response) => {
      if(response.data.Status === 'Error') {
        ...
        if (Array.isArray(response.data)) {
          ...
          setUserMeals([]); // Fallback to empty array if not
        }
      }
    }).catch(error => {
      console.error("Fetching user meals failed:", error);
      setUserMeals([]); // Ensure state remains an array even on error
    });
  }
